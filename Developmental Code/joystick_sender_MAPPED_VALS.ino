...
const int SERVO_MIN = 0; // 0 degress min servo deflection
const int SERVO_MAX = 180; // 180 degrees max servo deflection
const int MOTOR_MIN = 20; // Motor will start at digital value 20 to establish proper connection. (This means the motor should be ON upon startup!)
const int MOTOR_MAX = 170; // digitalRead gives values 0-255. The motor seems to "max-out" at 170.
...
void loop() {
  ...
  joystick_left_x_rudder = map(LEFT_x_joy, JOYSTICK_MIN, JOYSTICK_MAX, SERVO_MIN, SERVO_MAX);
  joystick_left_y_throttle = map(LEFT_y_joy, THROTTLE_MIN, JOYSTICK_MAX, MOTOR_MIN, MOTOR_MAX);

  // Mapping functions for RIGHT joystick.
  joystick_right_x_aileron = map(RIGHT_x_joy, JOYSTICK_MIN, JOYSTICK_MAX, SERVO_MIN, SERVO_MAX);
  joystick_right_y_elevator = map(RIGHT_y_joy, JOYSTICK_MIN, JOYSTICK_MAX, SERVO_MAX, SERVO_MIN);
